...
CzNMNTracker::CzNMNTracker(std::string strWalletFile)
{
...
CzNMNTracker::~CzNMNTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzNMNTracker::Init()
{
...
bool CzNMNTracker::Archive(CMintMeta& meta)
{
...
bool CzNMNTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzNMNTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzNMNTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzNMNTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzNMNTracker::GetSerialHashes()
{
...
CAmount CzNMNTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzNMNTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzNMNTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzNMNTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzNMNTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzNMNTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzNMNTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzNMNTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzNMNTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzNMNTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzNMNTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzNMNWallet* zNMNWallet)
{
    bool iszNMNWalletInitialized = (NULL != zNMNWallet);
    CMintMeta meta;
    ...
    if (! iszNMNWalletInitialized)
        zNMNWallet = new CzNMNWallet(strWalletFile);
    meta.isSeedCorrect = zNMNWallet->CheckSeed(dMint);
    if (! iszNMNWalletInitialized)
        delete zNMNWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzNMNTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzNMNTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzNMNTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzNMNTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzNMNTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzNMNTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzNMNWallet* zNMNWallet = new CzNMNWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zNMNWallet);
        }
        delete zNMNWallet;
        LogPrint("zero", "%s: added %d dznmn from DB\n", __func__, listDeterministicDB.size());
    }
