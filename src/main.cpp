...
#if defined(NDEBUG)
#error "NMN cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uNMN) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zNMN spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zNMN is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        if (!spend.HasValidSignature())
            return error("%s: V2 zNMN spend does not have a valid signature", __func__);

        libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
        if (tx.IsCoinStake())
            expectedType = libzerocoin::SpendType::STAKE;
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zNMN without the correct spend type. txid=%s", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
        ...
        return error("%s : zNMN spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zNMN spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-znmn");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zNMN mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
    ...
    int64_t nSubsidy = 0;
    if(chainActive.Tip()->nMoneySupply > Params().MaxMoneySupply() )
        return 0.0001 * COIN;
    ...
    if (nHeight < 100 ) {
        ...
        nSubsidy = 2.5 * COIN;
    } else if (nHeight >= 30000 && nHeight <= 34999) {
        nSubsidy = 5 * COIN;
    } else if (nHeight >= 35000 && nHeight <= 39999) {
        nSubsidy = 8 * COIN;
    } else if (nHeight >= 40000 && nHeight <= 44999) {
        nSubsidy = 13 * COIN;
    } else if (nHeight >= 45000 && nHeight <= 49999) {
        nSubsidy = 15 * COIN;
    } else if (nHeight >= 50000 && nHeight <= 54999) {
        nSubsidy = 18 * COIN;
    } else if (nHeight >= 55000 && nHeight <= 59999) {
        nSubsidy = 21 * COIN;
    } else if (nHeight >= 60000 && nHeight <= 64999) {
        nSubsidy = 24 * COIN;
    } else if (nHeight >= 65000 && nHeight <= 69999) {
        nSubsidy = 27 * COIN;
    } else if (nHeight >= 70000 && nHeight <= 74999) {
        nSubsidy = 26 * COIN;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from NMN
...
{
    RenameThread("nmn-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZNMNMinted()
{
...
void RecalculateZNMNSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zNMN supply
...
bool RecalculateNMNSupply(int nHeightStart)
{
...
{
    // NMN: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZNMNSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zNMN money supply in the block index
    if (!UpdateZNMNSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zNMN supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zNMNSpent: %s\n",
...
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert NMN to zNMN
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (chainActive.Height() < Params().LAST_POW_BLOCK() && fCheckPOW && !CheckProofOfWork(block.GetHash(), block.nBits))
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zNMN spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNMN serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zNMN stake block is less than allowed start height", __func__);

    if (CzNMNStake* zNMN = dynamic_cast<CzNMNStake*>(stake)) {
        CBlockIndex* pindexFrom = zNMN->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zNMN stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zNMN stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zNMN->GetValue()));
        if (nChecksum200 != zNMN->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zNMN->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZNMN() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zNMN fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> NMNInputs;
        std::vector<CTxIn> zNMNInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zNMNInputs.push_back(stakeIn);
            }else{
                NMNInputs.push_back(stakeIn);
            }
        }
        const bool hasNMNInputs = !NMNInputs.empty();
        const bool hasZNMNInputs = !zNMNInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNMNInputs)
                    ...
                    for (const CTxIn& NMNIn : NMNInputs){
                        if(NMNIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : NMNInputs) {
            ...
            // Now that this loop if completed. Check if we have zNMN inputs.
            if(hasZNMNInputs){
                for (const CTxIn& zNMNInput : zNMNInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zNMNInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-znmn");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zNMN mints and %d zNMN spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // NMN: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("znmn", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("znmn", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
