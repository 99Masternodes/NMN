...
bool GetKernelStakeModifier(uint256 hashBlockFrom, uint64_t& nStakeModifier, int& nStakeModifierHeight, int64_t& nStakeModifierTime, bool fPrintProofOfStake)
{
    ...
    int64_t nStakeModifierSelectionInterval = 1;//GetStakeModifierSelectionInterval();
...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzNMNStake(spend));
    } else {
        ...
        CNMNStake* NMNInput = new CNMNStake();
        NMNInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(NMNInput);
    }
