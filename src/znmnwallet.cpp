...
CzNMNWallet::CzNMNWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing znmn seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZNMNSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZNMNSeed_deprecated()) {
                    LogPrintf("%s: Updated zNMN seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old znmn seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of znmn wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzNMNWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZNMNCount(nCountLastUsed);
    else if (!walletdb.ReadZNMNCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzNMNWallet::Lock()
{
    seedMaster = 0;
}

void CzNMNWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzNMNWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZNMN(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzNMNWallet::LoadMintPoolFromDB()
{
...
void CzNMNWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzNMNWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzNMNWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->znmnTracker->HasPubcoinHash(pMint.first)) {
...
bool CzNMNWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZNMN(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
...
void CzNMNWallet::SeedToZNMN(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzNMNWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzNMNWallet::GenerateDeterministicZNMN(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzNMNWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzNMNWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzNMNWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
