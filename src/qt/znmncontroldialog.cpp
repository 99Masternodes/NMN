...
#include "ui_znmncontroldialog.h"
...
std::set<std::string> zNMNControlDialog::setSelectedMints;
std::set<CMintMeta> zNMNControlDialog::setMints;

bool CzNMNControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zNMNControlDialog::COLUMN_DENOMINATION || column == zNMNControlDialog::COLUMN_VERSION || column == zNMNControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zNMNControlDialog::zNMNControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zNMNControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zNMNControlDialog::~zNMNControlDialog()
{
    delete ui;
}

void zNMNControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zNMNControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzNMNControlWidgetItem* itemDenom(new CzNMNControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzNMNControlWidgetItem *itemMint = new CzNMNControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "The zNMN seed used to mint this zNMN is not the same as currently hold in the wallet";
            else
...
void zNMNControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zNMNControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzNMN_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzNMNControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zNMNControlDialog::GetSelectedMints()
{
...
void zNMNControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
