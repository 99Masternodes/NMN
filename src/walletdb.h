...
public:
    ...
    bool WriteZNMNSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZNMNSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZNMNSeed_deprecated(uint256& seed);
    bool EraseZNMNSeed();
    bool EraseZNMNSeed_deprecated();

    bool WriteZNMNCount(const uint32_t& nCount);
    bool ReadZNMNCount(uint32_t& nCount);
