...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 NMN at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zNMN send
enum ZerocoinSpendStatus {
    ZNMN_SPEND_OKAY = 0,                            // No error
    ZNMN_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZNMN_WALLET_LOCKED = 2,                         // Wallet was locked
    ZNMN_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZNMN_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZNMN_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZNMN_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZNMN_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZNMN_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZNMN_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZNMN_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZNMN_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZNMN_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZNMN_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZNMN_SPENT_USED_ZNMN = 14,                      // Coin has already been spend
    ZNMN_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZNMN_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zNMNBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZNMNOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fznmnAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzNMNTracker> znmnTracker;
    ...
    void setZWallet(CzNMNWallet* zwallet)
    {
        zwalletMain = zwallet;
        znmnTracker = std::unique_ptr<CzNMNTracker>(new CzNMNTracker(strWalletFile));
    }

    CzNMNWallet* getZWallet() { return zwalletMain; }
    ...
    void setzNMNAutoBackups(bool fEnabled)
    {
    ...
    /** zNMN reset */
    boost::signals2::signal<void()> NotifyzNMNReset;
