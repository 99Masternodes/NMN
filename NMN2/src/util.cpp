...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "nmn" is a composite category enabling all NMN-related debug output
            if (ptrCategory->count(string("nmn"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NMN
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NMN
// Mac: ~/Library/Application Support/NMN
// Unix: ~/.nmn
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NMN";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nmn.conf"));
    ...
    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override nmn.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nmnd.pid"));
