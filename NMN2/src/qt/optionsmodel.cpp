...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::NMN);
    ...
    if (!settings.contains("nAnonymizeNMNAmount"))
        settings.setValue("nAnonymizeNMNAmount", 1000);

    nAnonymizeNMNAmount = settings.value("nAnonymizeNMNAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeNMNAmount"))
        SoftSetArg("-anonymizenmnamount", settings.value("nAnonymizeNMNAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nmn.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeNMNAmount:
            return QVariant(nAnonymizeNMNAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizeNMNAmount:
            nAnonymizeNMNAmount = value.toInt();
            settings.setValue("nAnonymizeNMNAmount", nAnonymizeNMNAmount);
            emit anonymizeNMNAmountChanged(nAnonymizeNMNAmount);
            break;
        case CoinControlFeatures:
